...
export default class Cube {
   ...
   loadTexture() {
      this.cubeTexture = this.textureLoader.load('assets/gen_checkerboard-1.png');
      this.cubeMaterial = new THREE.MeshStandardMaterial({ map: this.cubeTexture });
   }
         ...
         if (child instanceof THREE.Group) {
            child.children.forEach((groupChild) => {
               if (groupChild instanceof THREE.Mesh) {
                  groupChild.castShadow = true;
                  groupChild.receiveShadow = true;
                  groupChild.material = this.cubeMaterial;
               }
            });
         }
